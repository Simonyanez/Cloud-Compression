...
def extract_info(subdata):
    ...
    bd_psnr = bj_delta(bpv, PSNR_Y, dbpv, dPSNR_Y, mode=0)
    bd_rate = bj_delta(bpv, PSNR_Y, dbpv, dPSNR_Y,  mode=1)
    return PSNR_Y, bpv, dPSNR_Y, dbpv,bd_psnr,bd_rate
...
def PSNR_vs_bpv_plot(data):
    ...
    for num in num_of_points:
        ...
        for i, bsize in enumerate(bsizes):
            ...
            PSNR_Y, bpv, dPSNR_Y, dbpv, bd_psnr,bd_rate = extract_info(blockpoint_data)
