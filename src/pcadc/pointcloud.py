...
class Sampler:
    ...
    def _build_strats(self, variances: np.ndarray, bins: np.ndarray, blocks: List[Block]):
        bin_indices = np.digitize(variances, bins) - 1
        strata = [[] for _ in range(len(bins) - 1)]
        for i, b_bin in enumerate(bin_indices):
            # Clip index to valid range of strata list
            idx = min(max(b_bin, 0), len(strata) - 1)
            strata[idx].append(blocks[i])
        return strata
