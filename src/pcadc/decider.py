...
class Decider:
    ...
    @profile
    def __call__(self, q_step: int, coeffs_container: CoeffsContainer, r=0.85) -> Tuple[GraphMetadata, np.ndarray, float]:
        self.lagrange_mult = r * q_step**2
        self.q_step = q_step
        logger.info(f"Starting RDO with q_step={q_step} and lambda={self.lagrange_mult:.4f}.")
    ...
    def _RDcost(self, Coeffs: np.ndarray):
        ...
        if self.mode == "0":
            ...
            logger.error(f"Invalid mode: {self.mode}. Skipping cost calculation.")
            return float('inf')
        ...
        logger.debug(f"Calculated RD cost. Quantization Error: {qerror:.4f}, Sparsity: {sparsity}, Total Cost: {rd_cost:.4f}.")
        return rd_cost
    ...
    def _RDO(self, coeffs_container: CoeffsContainer) -> RDO_Decision:
        ...
        logger.info(f"Evaluating {len(coeff_id_pairs)} candidate coefficient-graph pairs.")
        for i, (coeff, graph_obj) in enumerate(coeff_id_pairs):
            res = self._RDcost(coeff)
            logger.info(f"Candidate {i+1}/{len(coeff_id_pairs)} (Graph ID: {graph_obj.graph_id}) has a total cost of {res:.4f}.")
        ...
        if self.mode == "0":
            if struct_coeff is not None:
                ...
                logger.debug("Applied structural coefficients for U and V channels.")
            else:
                warn_err = "Structural graph was not a candidate. Mode 0 may be compromised."
                logger.warning(warn_err)
                raise ValueError(warn_err)

        logger.info(f"RDO finished. Selected graph ID: {selected_metadata.graph_id} with a minimum cost of {min_cost:.4f}.")
