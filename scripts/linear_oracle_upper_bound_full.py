...
def main():
    ...
    print(f"\n[LINEAR ORACLE] Starting FULL Point Cloud Search.")
    ...
    for bsize in block_sizes:
        ...
        for q in q_steps:
            ...
            with Pool(cpu_count()) as pool:
                batch = []
                for res in tqdm(pool.imap_unordered(linear_oracle_worker, tasks), total=len(tasks), desc=f"B={bsize} Q={q}"):
                    batch.append(res)
                    if len(batch) >= 100:
                        conn = sqlite3.connect(DB_PATH)
                        ...
                        batch = []
                if batch:
                    conn = sqlite3.connect(DB_PATH); conn.executemany("INSERT OR REPLACE INTO results VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", batch); conn.commit(); conn.close()

    print("\n[COMPLETE] Full Linear Oracle search finished.")
