"""
Experiment E4: Full Multi-Stage Integrated Loop
===============================================
Evaluates the complete integrated loop combining:
  1. Gain-aware pruning (tau threshold decreases over iterations)
  2. Continuity annealing (gamma penalty increases over iterations)
  3. Dynamic centroid retraining on active high-gain blocks

Algorithm Schedule across T=4 Outer Iterations:
  Iter 0 (Warmup): gamma^(0) = 200, tau^(0) = 2.0%
  Iter 1:          gamma^(1) = 600, tau^(1) = 1.5%
  Iter 2:          gamma^(2) = 1200, tau^(2) = 1.0%
  Iter 3 (Final):  gamma^(3) = 2000, tau^(3) = 0.5%

At each step t:
  - Mask active blocks: M_i^(t) = (Gain_i > tau^(t))
  - Retrain centroids mu_k^(t) on active blocks
  - Run Viterbi under gamma^(t)
  - Compute full metrics (GFT%, Overhead, Net BPV, Centroid Separation, Active%)

Runtime: ~6-8 min for B=16, K=4.
"""
...
def main():
    ...
    section(f"E4: Full Multi-Stage Integrated Loop  (B={B}, K={K})")
    ...
    # Multi-stage schedule: (gamma, tau_pct)
    schedule = [
        (200.0, 2.0),
        (600.0, 1.5),
        (1200.0, 1.0),
    ...
    section("MULTI-STAGE INTEGRATED LOOP EXECUTION")
    print(f"\n{'Stage':>5} | {'γ penalty':>9} | {'τ threshold':>11} | {'Active%':>7} | "
          f"{'GFT%':>7} | {'Overhead':>10} | {'Net BPV':>10} | {'CentSep':>8}")
    print("-" * 88)

    h_history = []
    for stage, (gamma_t, tau_t) in enumerate(schedule):
        # 1. Build cost matrix with current centroids
        cost_matrix = build_cost_matrix(blocks, tangent_data, struct_map,
                                        centroids, slp, slw, g3d_raw,
                                        pc.V, pc.A, dm, lag, q, K)
        struct_total = cost_matrix[:, 0].sum()

        # 2. Compute gain & active mask
        ...
        # 3. Retrain centroids on active blocks
        if n_active >= K_hybrid * 3:
            active_grads = g3d_norm[active_mask]
            centroids, _ = spherical_kmeans(active_grads, K_hybrid)

        # Re-build cost matrix with retrained centroids
        cost_matrix = build_cost_matrix(blocks, tangent_data, struct_map,
                                        centroids, slp, slw, g3d_raw,
                                        pc.V, pc.A, dm, lag, q, K)

        # 4. Run Viterbi under gamma_t
        vl = run_viterbi(cost_matrix, K, gamma_t, nw)
        ...
        print(f"{stage+1:>5} | {gamma_t:>9.1f} | {tau_t:>10.1f}% | {active_pct:>6.1f}% | "
              f"{sav:>6.3f}% | {oh:>10.5f} | {net:>+10.5f} | {sep:>8.4f}")

        h_history.append((stage+1, gamma_t, tau_t, active_pct, sav, oh, net, sep))

    section("FINAL DIAGNOSTIC VERDICT")
    init_net = h_history[0][6]
    final_net = h_history[-1][6]
    log(f"Stage 1 Net BPV (γ=200, τ=2.0%): {init_net:+.5f}")
    log(f"Stage 4 Net BPV (γ=2000, τ=0.5%): {final_net:+.5f}")
    log(f"Overall Multi-Stage Net Progress: {final_net - init_net:+.5f}")
