"""
Oracle-Initialized EM Codebook Training
========================================
Replaces Differential Evolution with a two-part M-step:
  1. Closed-form least-squares slope update (analytical, exact)
  2. 2D coarse→fine grid search over (slp, slw) with slope fixed

E-step adds a sequential transition penalty to encourage spatial coherence
and reduce the overhead entropy of the cluster assignment sequence.

Key insight from oracle DB analysis:
  - fit_rmse drives gain magnitude (corr +0.687 with gain)
  - 3 well-placed modes retains ~82% of oracle gain
  - Overhead break-even requires H(X|X_prev) well below H(X) max
  - Oracle K-Means on (sx,sy,sz,slp*,slw*) gives a meaningful initialization

EM Loop:
  Repeat until convergence:
    E-step: for each block, evaluate RD_cost(block, k) for all k in parallel
            then sequentially apply transition penalty gamma * (-log P(k|k_prev))
    M-step: for each cluster k with assigned blocks:
            (a) slope_k = lstsq(V_norm, Y) over assigned blocks  [closed-form]
            (b) (slp_k, slw_k) = coarse 10x10 grid -> 5x5 refinement  [grid search]

The slope IS dynamic — it changes every M-step as block membership changes.
Oracle K-Means provides iteration-0 starting values only.

Usage:
  cd /path/to/Cloud-Compression
  python scripts/oracle_em_codebook.py \\
    --block_size 8 --clusters 4 --q_step 24 \\
    --gamma 0.0 --max_iters 10 \\
    --out_dir experiments/oracle_em_test

  # With transition penalty:
  python scripts/oracle_em_codebook.py \\
    --block_size 8 --clusters 4 --q_step 24 \\
    --gamma 500.0 --max_iters 10 \\
    --out_dir experiments/oracle_em_gamma500
"""
...
# ---------------------------------------------------------------------------
# Path setup (same pattern as all other scripts)
# ---------------------------------------------------------------------------
...
# ===========================================================================
# STEP 1 — Oracle K-Means Codebook Initialization
# ===========================================================================

def load_oracle_data(db_path: Path, bsize: int) -> dict:
    """
    Load oracle DB records for a given block size.
    Returns arrays: slope (Nx3), slp* (N,), slw* (N,), gain (N,), rmse (N,)
    """
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    # Pool all Q-steps — the gain-weighted K-Means should work across Q values
    cur.execute("""
        SELECT slope_x, slope_y, slope_z, best_p, best_w, gain, fit_rmse
        FROM results
        WHERE bsize=? AND gain > 0
    """, (bsize,))
    rows = cur.fetchall()
    conn.close()

    if not rows:
        raise ValueError(f"No oracle data for bsize={bsize}")

    data = np.array(rows, dtype=np.float64)
    return {
        "slopes": data[:, :3],     # (N, 3)  [sx, sy, sz] in PCA-rotated space
        "slp":    data[:, 3],       # (N,)    optimal self-loop percentage
        "slw":    data[:, 4],       # (N,)    optimal self-loop weight
        "gain":   data[:, 5],       # (N,)    RD gain used as sample weight
        "rmse":   data[:, 6],       # (N,)    fit RMSE (for diagnostics)
    }


def gain_weighted_kmeans(features: np.ndarray, weights: np.ndarray,
                          k: int, n_init: int = 5, max_iter: int = 100,
                          seed: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    """
    Gain-weighted K-Means on feature matrix.
    Features are normalized per-dimension before clustering.
    Returns: (centroids in original space, centroid_stds for diagnostics)
    """
    ...
    X = (features - feat_min) / feat_range

    # Weighted K-Means++ initialization
    w = weights / weights.sum()
    centers_idx = [rng.choice(len(X), p=w)]
    for _ in range(k - 1):
        dists = np.array([np.min([np.sum((x - X[ci]) ** 2) for ci in centers_idx])
                          for x in X])
        dists_w = dists * w
        dists_w /= dists_w.sum()
        centers_idx.append(rng.choice(len(X), p=dists_w))
    ...
    for trial in range(n_init):
        if trial == 0:
            ...
            # Random restart from weighted sampling
            centers = X[rng.choice(len(X), size=k, replace=False, p=w)].copy()
        ...
        for _ in range(max_iter):
            ...
            for j in range(k):
                mask = (labels == j)
                if mask.sum() == 0:
                    # Dead cluster: reinitialize at highest-weight unassigned point
                    furthest = np.argmax(np.min(dists[:, [m for m in range(k) if m != j]], axis=1))
        ...
        inertia = np.sum(w * np.min(dists, axis=1))
        if inertia < best_inertia:
            best_inertia = inertia
            best_centers = centers.copy()
            best_labels = labels.copy()

    # De-normalize centroids back to original space
    best_centers_orig = best_centers * feat_range + feat_min
    return best_centers_orig, best_labels
...
# ===========================================================================
# STEP 2 — E-step: Parallel RD evaluation + Sequential transition penalty
# ===========================================================================
...
def _evaluate_block_costs_worker(block_id, Vblock, Ablock, metadata,
    ...
    Returns cost vector of length K.
    Cluster 0 is always structural (uses precomputed coefficients).
    """
    ...
    for k, slope in enumerate(slopes):
        if k == 0:
            # Structural baseline — use precomputed coefficients
...
def e_step(blocks: List, structural_coeffs_map: dict,
           ...
           iteration: int, gamma_warmup_iters: int = 2) -> Tuple[np.ndarray, np.ndarray]:
    """
    E-step: evaluate all K clusters per block in parallel, then apply
    transition penalty sequentially.

    gamma warmup: gamma=0 for first gamma_warmup_iters iterations so clusters
    form naturally before the transition penalty locks assignments in.

    Returns:
        new_labels: (N,) cluster assignments
        cost_matrix: (N, K) full RD cost matrix (for diagnostics)
    """
    # Ramp gamma
    effective_gamma = 0.0 if iteration <= gamma_warmup_iters else gamma
    ...
    results = Parallel(n_jobs=-1)(
        ...
        for t in tqdm(tasks, desc=f"  E-step (iter {iteration}, γ={effective_gamma:.0f})")
    )

    cost_matrix = np.array(results)  # (N, K)
    N, K = cost_matrix.shape
    new_labels = np.zeros(N, dtype=int)

    if effective_gamma == 0.0 or prev_labels is None:
        # Pure RD cost assignment — fully parallel
        new_labels = np.argmin(cost_matrix, axis=1)
    else:
        # Sequential pass with flat transition penalty (Potts model)
        # Any cluster change costs effective_gamma (similar to beta in the codebase)
        new_labels[0] = np.argmin(cost_matrix[0])
        for i in range(1, N):
            adj_costs = cost_matrix[i].copy()
            mask = np.ones(K, dtype=bool)
            mask[new_labels[i-1]] = False
            adj_costs[mask] += effective_gamma
...
# ===========================================================================
# STEP 3 — M-step: Closed-form slope + 2D grid search for (slp, slw)
# ===========================================================================

def _compute_slope_closed_form(Vblocks: List[np.ndarray],
                                Ablocks: List[np.ndarray]) -> np.ndarray:
    """
    Analytically compute the optimal slope for a cluster given its assigned blocks.

    The slope minimizes: Σ_i || V_norm_i @ slope - Y_i ||^2
    Solution: slope = lstsq(stack(V_norm_i), stack(Y_i))

    V_norm_i is the PCA-rotated + centered spatial coordinates (same as Approximator).
    Y_i is the luminance channel (Ablock[:,0]).
    """
    approximator = Approximator()
    ...
    V_all = np.vstack(V_stack)   # (total_points, 3)
    Y_all = np.concatenate(Y_stack)  # (total_points,)

    # Least squares: slope = (V^T V)^{-1} V^T Y
    slope, _, _, _ = np.linalg.lstsq(V_all, Y_all, rcond=None)
    return slope  # shape (3,)
...
def _eval_grid_point(Vblocks_list, Ablocks_list, metadata_list,
                     ...
                     q_step, lagrange_proportional, decider_mode):
    """
    Worker: evaluate total RD cost of one (slp, slw) grid point over all assigned blocks.
    Parallelized per grid point — slope is fixed.
    """
    os.environ["OMP_NUM_THREADS"] = "1"
    os.environ["MKL_NUM_THREADS"] = "1"
...
def _evaluate_grid_parallel(Vblocks_list, Ablocks_list, metadata_list,
    ...
    """
    Evaluate a (slp_vals x slw_vals) grid in parallel — one job per grid point.
    Returns total_costs matrix of shape (len(slp_vals), len(slw_vals)).
    """
...
def m_step(blocks: List, labels: np.ndarray,
           ...
           vertices: np.ndarray, attributes: np.ndarray,
           q_step: int, lagrange_proportional: float, decider_mode: str,
           min_cluster_size: int = 10, sample_rate: float = 1.0) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    M-step: update (slope, slp, slw) for each cluster given current assignments.

    Cluster 0 is always structural — never updated.
    For clusters 1..K:
      (a) slope updated via closed-form least squares over assigned blocks
      (b) (slp, slw) found via coarse 10x10 grid + 5x5 fine refinement

    Cluster revival: if a hybrid cluster has < min_cluster_size blocks,
    it keeps its previous parameters (not updated from insufficient data).
    """
    K = len(slopes)
    ...
    for k in range(1, K):  # Skip cluster 0 (structural)
        ...
        # Collect block data (with sub-sampling)
        ...
        # (a) Closed-form slope — analytical, no iterations
        ...
        # (b) Coarse grid search for (slp, slw) with slope fixed — parallel over grid points
        ...
        # (c) Fine refinement around coarse winner — parallel over grid points
...
def compute_metrics(blocks: List, labels: np.ndarray, cost_matrix: np.ndarray,
    ...
    # Entropy of label sequence (= overhead if using marginal model)
    probs = cluster_sizes / cluster_sizes.sum()
    entropy = -np.sum(probs[probs > 0] * np.log2(probs[probs > 0]))

    # Conditional entropy H(X_t | X_{t-1}) — what a Markov coder would pay
    ...
    overhead_marginal  = entropy  * N / total_v   # bpv if marginal coder
    overhead_markov    = h_cond   * N / total_v   # bpv if Markov coder

    # Gain vs structural (cluster 0)
...
def converged(history: list, window: int = 3,
    ...
    """
    Stop if:
      (a) relative cost change < cost_tol over last `window` iterations, OR
      (b) average Hamming distance between label sequences < hamming_tol * N
    """
...
def main():
    parser = ArgumentParser(description="Oracle-initialized EM codebook training")
    ...
    parser.add_argument("--gamma",       type=float, default=0.0,
                        help="Transition penalty weight (0=disabled)")
    parser.add_argument("--max_iters",   type=int,   default=10)
    parser.add_argument("--min_iters",   type=int,   default=3)
    parser.add_argument("--gamma_warmup",type=int,   default=2,
                        help="Iterations before gamma penalty activates")
    parser.add_argument("--out_dir",     type=str,   required=True)
    parser.add_argument("--config",      type=str,   default="config/base_config.yaml")
    parser.add_argument("--oracle_db",   type=str,   default="results/linear_oracle_upper_bound.db",
                        help="Path to linear oracle SQLite DB")
    parser.add_argument("--sample_rate", type=float, default=1.0,
                        help="M-step training blocks sub-sampling rate (0.0 to 1.0)")
    args = parser.parse_args()

    n_hybrid = args.clusters - 1  # Cluster 0 is always structural
    if n_hybrid < 1:
        raise ValueError("--clusters must be >= 2 (at least 1 structural + 1 adaptive)")

    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    # ── Load config + point cloud ──────────────────────────────────────────
    ...
    print(f"[Setup] Q-step: {args.q_step}  Gamma: {args.gamma}  Max iters: {args.max_iters}")
    ...
    min_cluster_size = max(5, int(0.01 * len(blocks)))  # 1% of blocks or 5, whichever larger
    ...
    for iteration in range(1, args.max_iters + 1):
        ...
        labels, cost_matrix = e_step(
            ...
            iteration=iteration,
            gamma_warmup_iters=args.gamma_warmup
        )
        ...
        for k in range(1, len(slopes)):
            if cluster_sizes[k] < min_cluster_size:
                ...
                # Find block with highest gain potential (structural cost - best dynamic cost)
                gain_potential = cost_matrix[:, 0] - np.min(cost_matrix[:, 1:], axis=1)
                # Among blocks NOT currently in k
        ...
        print(f"  Rate reduction: {metrics['rate_reduction_pct']:>10.2f}%  vs structural baseline")
    ...
    final = history[-1]
    print(f"\n{'='*60}")
    print(f"FINAL RESULTS  (B={args.block_size}  C={args.clusters}  Q={args.q_step}  γ={args.gamma})")
    print(f"{'='*60}")
    print(f"  Iterations run:        {len(history)}")
    print(f"  Coeff rate reduction:  {final['rate_reduction_pct']:.2f}%")
    print(f"  Overhead (marginal):   {final['overhead_bpv_marginal']:.4f} bpv")
    print(f"  Overhead (Markov):     {final['overhead_bpv_markov']:.4f} bpv")
    print(f"  Active clusters:       {final['active_clusters']} / {len(slopes)}")
