"""
Multi-Stage Decoupled Spatial EM Optimization Sweep
===================================================
Phase 1: Unconstrained GFT EM Codebook Training (gamma = 0) with MSAE convergence monitoring.
Phase 2: Frozen One-Shot Spatial Viterbi Sweep (using GFT RD cost matrix).
...
# Helper to calculate MSAE
def calculate_msae(labels, normals, K):
    centroids = np.zeros((K, 3))
    # Calculate centroids
    for k in range(K):
        idx = np.where(labels == k)[0]
        if len(idx) == 0:
            centroids[k] = np.array([0.0, 0.0, 1.0])
            continue
        sum_n = np.zeros(3)
        # Seed alignment direction
        ref = normals[idx[0]]
        for i in idx:
            n = normals[i]
            sign = np.sign(np.dot(n, ref))
            if sign == 0:
                sign = 1.0
            sum_n += sign * n
...
results_table = []
results_table.append("# Decoupled Spatial GFT EM Sweep Report\n")
...
for K in Ks:
    ...
    # Run EM training at gamma = 0
    for it in range(1, 21):
        print(f"  Iteration {it:2d}/20...")
        
        # 1. E-step: GFT cost evaluation (gamma = 0)
        cost_matrix = []
        ...
        labels = np.argmin(cost_matrix, axis=1)
        
        # Revival check for empty clusters
        min_cluster_size = max(5, int(0.01 * N))
        sizes = np.bincount(labels, minlength=K)
        for k in range(1, K):
            if sizes[k] < min_cluster_size:
                gain = cost_matrix[:, 0] - np.min(cost_matrix[:, 1:], axis=1)
                candidates = np.where(labels != k)[0]
                if len(candidates) > 0:
                    labels[candidates[np.argmax(gain[candidates])]] = k
                    
        # Compute MSAE convergence monitor
        msae, _ = calculate_msae(labels, normals, K)
        delta_msae = np.abs(msae - prev_msae)
        print(f"    MSAE: {msae:.6f} | Delta MSAE: {delta_msae:.8f}")
        
        if delta_msae < 1e-5 and converged_iter == -1:
            converged_iter = it
            print(f"    [Convergence] MSAE converged at iteration {it}!")
            
        prev_msae = msae
        prev_labels = labels.copy()
        
        # 2. M-step: regression + loop parameters search
        # Update slopes
        new_slopes = slopes_2d.copy()
        for k in range(1, K):
            mask = (labels == k)
            n_ass = mask.sum()
            if n_ass < min_cluster_size:
                continue
            ...
            if X_list:
                ...
                new_slopes[k] = s2d_new
                
        slopes_2d = new_slopes
        
        # Update slp, slw loop parameters on a sample
        for k in range(1, K):
            mask = (labels == k)
            n_ass = mask.sum()
            if n_ass < min_cluster_size:
    ...
    # Compute final MSAE and centroids
    final_msae, _ = calculate_msae(labels, normals, K)
    print(f"\n[Phase 1 Done] K={K} converged/ended at iteration {converged_iter}. Final MSAE: {final_msae:.6f}")

    # Precompute GFT cost matrix using the frozen converged codebook
    print("Evaluating GFT cost matrix with frozen unconstrained codebook...")
    gft_cost_matrix = []
    for i, block in enumerate(blocks):
        block.init_data(pc.V, pc.A)
        Vblock = block.Vblock
    ...
    for gamma0 in gammas:
        ...
        # normal alignment weights along Morton sequence: w_i = |n_i . n_{i-1}|
        normal_weights = np.ones(N)
        for i in range(1, N):
            normal_weights[i] = np.abs(np.dot(normals[i], normals[i-1]))
            
        # Viterbi trellis dynamic programming solver using GFT costs
        dp = np.zeros((N, K))
        paths = np.zeros((N, K), dtype=int)
        dp[0] = gft_cost_matrix[0]
            
        for i in range(1, N):
            w = normal_weights[i]
            for k in range(K):
                # Transition cost: 0 if same, gamma0 * |n_i . n_{i-1}| if different
                transition_costs = dp[i-1] + gamma0 * w * (np.arange(K) != k)
                best_prev = np.argmin(transition_costs)
                dp[i, k] = gft_cost_matrix[i, k] + transition_costs[best_prev]
                paths[i, k] = best_prev
        ...
        # Record results
        results_table.append(f"| {K} | {gamma0:.1f} | {converged_iter} | {final_msae:.6f} | {savings_pct:.3f}% | {entropy:.4f} | {overhead:.5f} | {net_bpv:+.5f} | {switches} |")

# Write to markdown file
artifact_dir = Path("/home/simao/.gemini/antigravity-cli/brain/bea95583-c565-4fdd-b212-641f0ae23deb")
artifact_dir.mkdir(parents=True, exist_ok=True)
report_file = artifact_dir / "decoupled_sweep_results.md"
