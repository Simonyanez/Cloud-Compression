...
\subsubsection{Agrupamiento Tasa-Distorsión}

Sobre este conjunto se busca identificar un subconjunto óptimo de centroides que minimice el costo global de compresión, considerando simultáneamente la calidad de reconstrucción (distorsión) y la eficiencia de codificación (tasa de bits). A diferencia de los métodos de clusterización tradicionales que minimizan únicamente distancias geométricas, se implementa un algoritmo de \textbf{agrupamiento Tasa-Distorsión (RD Clustering)} que optimiza directamente el compromiso entre estos dos objetivos.
...
donde:
\begin{itemize}
    \item \textbf{Distorsión} \(D_j(i)\): Representa el error de cuantización de los coeficientes de la GFT adaptativa \(\mathcal{F}_j\) aplicada al bloque \(i\). Esta distorsión mide la pérdida de calidad resultante de la aproximación y cuantización de los coeficientes espectrales.
    
    \item \textbf{Tasa} \(R_j(i)\): Estima el número de bits necesarios para codificar los coeficientes \(\mathcal{F}_j(f_i)\), donde \(f_i\) es la señal de luminancia del bloque \(i\).
    
    \item \textbf{Multiplicador de Lagrange} \(\lambda\): Controla el balance entre distorsión y tasa. Valores altos de \(\lambda\) priorizan la reducción de bits a costa de mayor distorsión, mientras que valores bajos favorecen la calidad de reconstrucción.
\end{itemize}
...
\paragraph{Inicialización:} Se define un conjunto de \(K\) centroides iniciales. El primer centroide (\(j=0\)) se fija como un vector de pendiente nula \(\mathbf{c}_0 = [0, 0, 0]^\top\), que corresponde a la GFT estructural basada únicamente en conectividad geométrica. Los restantes \(K-1\) centroides se inicializan a partir de las pendientes de bloques seleccionados aleatoriamente o mediante k-means sobre \(\tilde{\boldsymbol{\beta}}\).

\paragraph{Asignación Estocástica:} En cada iteración, para cada bloque \(i\), se calcula el costo RD \(J_j(i)\) que resultaría de asignarlo a cada uno de los \(K\) clusters. Para el cluster estructural (\(j=0\)), se utiliza la GFT precomputada. Para los clusters dinámicos (\(j \geq 1\)), se construye un grafo basado en atributos utilizando el centroide \(\mathbf{c}_j\) y los parámetros de auto-bucles configurados.

Para evitar la convergencia prematura a mínimos locales, la asignación no es determinista. Se emplea un esquema de \textbf{Simulated Annealing}, donde la probabilidad de asignar el bloque \(i\) al cluster \(j\) es:

\begin{equation}
    P(j|i) = \frac{e^{-J_j(i)/T}}{\sum_{k=0}^{K-1} e^{-J_k(i)/T}}
\end{equation}

La temperatura \(T\) disminuye gradualmente según un esquema de enfriamiento geométrico \(T \leftarrow \alpha T\), permitiendo exploración inicial del espacio de soluciones y convergencia hacia óptimos al final del proceso.

\paragraph{Actualización de Centroides:} Una vez reasignados todos los bloques, cada centroide dinámico \(\mathbf{c}_j\) (\(j \geq 1\)) se recalcula como el promedio ponderado de las pendientes de los bloques asignados a ese cluster:

\begin{equation}
    \mathbf{c}_j \leftarrow (1-\eta)\mathbf{c}_j + \eta \cdot \frac{1}{|C_j|} \sum_{i \in C_j} \tilde{\beta}_i
\end{equation}

donde \(\eta\) es una tasa de aprendizaje que controla la velocidad de actualización de los centroides y \(C_j\) denota el conjunto de bloques asignados al cluster \(j\). El centroide estructural \(\mathbf{c}_0\) permanece fijo durante todo el proceso.

\paragraph{Criterios de Convergencia:} El algoritmo se detiene cuando se alcanza el número máximo de iteraciones o cuando los cambios en las asignaciones de clusters entre iteraciones consecutivas caen por debajo de un umbral predefinido.
...
\section{\textit{Overhead} de Decisión}\label{sec:overhead_method}
    % Offline(train set) o Online
    El \textit{overhead}, en este contexto, corresponde a la porción de codigo reservada para las instrucciones de compresión y su representación económica es uno de los principales desafíos de este trabajo. Para la implementación óptima del presente algoritmo, es relevante que el codificador conozca las aproximaciones lineales posibles que representen la dinámica del color de distintos bloques. Esto se realiza a través de un proceso de entrenamiento, donde un subconjunto de diversidad estadística de bloques pertenecientes a una secuencia, determinan el conjunto de coeficientes representativos de la dinámica de los atributos del bloque. \\ 
...
\section{Etapa de Decodificación}\label{sec:decode_method}
   ...
   % INFORMATION THEORY ANALYSIS AND METRICS ARE MISSING 
