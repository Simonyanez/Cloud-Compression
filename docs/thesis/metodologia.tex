\chapter{Metodología}
% La metodología está bien organizada y cubre todo el “pipeline”; sin embargo, mejoraria si (i) abres el capítulo con un breve mapa subcapitulos y sus objetivos, (ii) haces referencias cruzadas explícitas a secciones previas cuando dependes de conceptos (Morton, GFT, pesos del grafo), (iii) refuerzas la consistencia de los terminos y tipográfia (Qset/Qstep, vectores en negrita o con flecha, uso del español en títulos), y (iv) sustituyes (“figura anterior/siguiente”) por referencias numeradas. Con esos ajustes (más la eliminación de repeticiones y pequeños comentarios mencionados posteriormente), el flujo se vuelve más autocontenido y facil de leer para el lector no experto.
% Compactación de la energía será relevante mencionar en marco teórico
En esta sección se detalla por completo la metodología del algoritmo propuesto, organizada en distintas secciones fundamentales. Primero, se explica la reconstrucción de la base metodológica trabajada en la sección \ref{sec:pcac_geometric}, lo que incluye la partición de la nube (\ref{sec:pc_partition}) y la construcción del grafo estructural (\ref{sec:structural_graph}). Luego, se detalla la estrategia desarrollada para adaptar los grafos aprovechando la información de luminancia, con el objetivo de generar bases de transformada de grafos (GFT) más informativas (\ref{sec:luminance_clustering}, \ref{sec:attribute_graph}, \ref{sec:method_gft}). Posteriormente, se describe el proceso de codificación utilizando optimización tasa-distorsión para seleccionar por bloque los mejores coeficientes, optimizar el codificador entrópico e incluir el overhead necesario para las instrucciones del método (\ref{sec:rdo_method}, \ref{sec:rlgr_method}, \ref{sec:overhead_method}). Finalmente, se describen los procesos de decodificación y las métricas de rendimiento asociadas (\ref{sec:decode_method}, \ref{sec:metrics_method}).


\section{Esquema de compresión} \label{sec:copmresion_scheme}
    Privilegiando la comprensión del lector, las figuras \ref{fig:data_pipeline} y \ref{fig:compresion_scheme} muestran de forma general las etapas del esquema de compresión. El primero muestra el flujo de los datos geométricos y dónde está ubicado, precisamente, el algoritmo propuesto en el esquema completo de compresión. El segundo muestra el esquema de compresión con la descripción de las etapas utilizadas para el presente trabajo.
    ...
    En la figura \ref{fig:data_pipeline}, se muestra la geometría de la nube de puntos como conocida y sin pérdida tanto por el codificador como por el decodificador de atributos. Lo anterior es fundamental, pues permite a la implementación asumir la geometría y utilizarla de forma agnóstica a su compresión, sin agregar información adicional al flujo de bits. Entendiendo esto, la figura \ref{fig:compresion_scheme} muestra el esquema de codificación propuesto para los atributos de color de la nube de puntos.
    ...
    \begin{figure}
        ...
        \label{fig:compresion_scheme}
    \end{figure}

\section{Datos Utilizados}\label{sec:datasets}
% - Describir el dataset utilizado (nombre, resolución, fuente, etc.)
    Lo primero a mencionar son las características de los datos utilizados para la evaluación de la metodología, dado que esto condiciona el procesamiento y el análisis posterior. \\

    Existen distintas formas de obtener datos de nubes de puntos, que limitan la resolución y caracterizan por tanto su cantidad máxima de puntos y las referencias espaciales máximas que se pueden representar. En este trabajo se utilizaron dos bases de datos clásicas y ampliamente estudiadas en el campo de compresión de nubes de puntos.\\

    % Indica explícitamente que las coordenadas se cuantizan a enteros y su rango/resolución (sin valores si aún no están), o remite a una subsección donde se precise.
    De forma general, ambas fuentes de datos contienen nubes de puntos descritas de la misma manera con metodologías de obtención distintas. Lo anterior implica diferencias en su contenido condicionando el análisis y conclusiones posteriores. Sin embargo, la representación es la misma y consiste en un conjunto de puntos espacialmente ubicados según ejes cartesianos ($x$, $y$, $z$) restringidos por una grilla rectangular en 3D que, sin pérdida de generalidad, se asume que pertenece a un enmallado de números enteros \cite{deon2017voxelized}. Se dice de manera formal que cada uno de estos puntos cartesianos se ubica en la grilla a través de un \textit{voxel}, que sería el equivalente a un pixel para un objeto tridimensional. En este contexto, se habla de que ambas fuentes de datos poseen nubes de puntos voxelizadas, cuya cantidad de puntos es no homogénea. Es importante destacar que los \textit{voxeles} pueden estar tanto ocupados como desocupados, a diferencia de una imagen que posee información de color y señal en cada uno de sus píxeles, lo que justifica la búsqueda de estructuras no convencionales para la representación de la señal.\\
    
    \subsection{\textit{8i Voxelized Full Bodies}}
        Para este primer caso, especialmente relevante por su complejidad y diversidad de fuentes, se trata de cuatro secuencias dinámicas de un sujeto en movimiento ocupando distintas vestimentas. Las secuencias están tipificadas como \textit{Long Dress}, \textit{Loot}, \textit{Red and Black} y \textit{Soldier} , cada una capturada a treinta cuadros por segundo en un periodo de diez segundos, lo que considera aproximadamente 300 imágenes por secuencia.
        ...
        Estos datos fueron obtenidos a través de cuarenta y dos cámaras RGB configuradas en catorce agrupaciones. Cada una de estas agrupaciones actúa lógicamente como una cámara de cuatro canales incluyendo la reflectancia. Se ocupa la misma resolución espacial para cada una de las secuencias: un cubo de profundidad diez, es decir, una grilla de $2^{10} = 1024$ posiciones por orientación, entregando un total de $2^{10^{3}} \approx 1.07e9$ posibles posiciones. De todas maneras, para cada secuencia el cubo es escalado para ser el cubo delimitador más pequeño posible que contenga al sujeto. Es importante destacar que como típicamente la altura del sujeto es la dimensión más grande de la secuencia, la proporcionalidad en sistema internacional de los vóxeles estará dada por la siguiente expresión: 
        \begin{equation}
         \text{proporción} \frac{[m]}{[voxel]} \approx \frac{\text{altura}\;[m]}{1024 \; [voxels]}
        \end{equation}

        Para generar cierto tipo de intuición, un sujeto de $1.8$ metros implica aproximadamente $1.75$ milímetros por vóxel. Lo que implica que las nubes de puntos están pensadas para ser percibidas a cuerpo completo.\\
        % - Justificar la elección del conjunto de datos
        En resumen, la relevancia específica de este conjunto está principalmente en la diversidad de color, la continuidad de la secuencia y la compresión en contexto de cuerpo completo.

    \subsection{\textit{Microsoft Voxelized Upper Bodies}}
        Este conjunto de datos a diferencia del anterior, consiste en imágenes de los troncos superiores de cinco sujetos catalogados como: \textit{Andres}, \textit{David}, \textit{Phil}, \textit{Ricardo} y \textit{Sara}. Cada uno de estos sujetos fueron capturados en siete y diez segundos completando dos secuencias. Las secuencias más cortas están capturadas con profundidad nueve, es decir, una grilla de $2^{9^{3}}$, mientras las más extensas con una profundidad de diez.\\
        La obtención de estos datos se hizo mediante cuatro cámaras RGBD a treinta cuadros por segundo entregando un total de 210 y 300 imágenes para cada versión respectiva. Como es razonable, la profundidad cambia la resolución de la nube de puntos donde aproximadamente los vóxeles de profundidad nueve corresponden a $1.5\;[mm³]$ mientras que las de profundidad diez corresponden a $0.75\;[mm³]$ \cite{loop2016microsoft}. 
        ...
        \end{figure}
        % - Justificar la elección del conjunto de datos
        Este conjunto se caracteriza por una resolución más fina y es relevante para observar el desempeño del algoritmo a corta distancia. Para análisis cualitativos será fundamental observar como se diferencia el desempeño del método entre este conjunto de datos y el anterior. 

    Finalmente, es importante mencionar que ambas bases de datos presentan las nubes de puntos indexadas según el orden de Morton explicado en la sección \ref{sec:morton_order}. Este orden privilegia la cercanía indexada de aquellos puntos que poseen mayor cercanía representándolos como una lista unidimensional.
    
\section{Partición de la Nube de Puntos} \label{sec:pc_partition}
    ...
    Luego, se operan las coordenadas según dicho tamaño de bloque con el objetivo de determinar como los puntos se ubican dentro de estas nuevas celdas tridimensionales. Matemáticamente se hace una cuantización escalar la cuál nos entrega la frontera más cercana para los bloques del tamaño elegido. Sea $v_i$ la posición del punto para el índice $i$.
     ...
     Donde los puntos que pertenecerán a cierto bloque siempre y cuando $\Delta_i = 0$ para las distintas comparaciones entre puntos. Esto nos entregará un conjunto de índices, que gracias al orden de Morton, son secuenciales y, por tanto, se representan por su índice inicial y final.\\
...
\section{Construcción del Grafo Estructural}\label{sec:structural_graph}
...
\section{Análisis de Patrones de Atributos} \label{sec:luminance_clustering}
    % AUN EN DESARROLLO
    Hasta el momento, toda la metodología ha trabajado con los datos espaciales de la nube de puntos. Sin embargo, el objetivo radica en la compresión de los atributos incluyendo información relevante de la señal. Para ello debe existir un balance entre su impacto en compresión y su costo de transmisión lo que obliga a plantear formas económicas de incluir información de la señal.
    %A continuación se  métodos clásicos de codificación basado en transformadas (\ref{sec:transform_based_coding}).\\
    ...
    Por supuesto, para poder lograr esto se hace indispensable la normalización de los bloques para que las representaciones sean congruentes bloque a bloque. Por tanto, el primer paso es la normalización posicional y rotacional del bloque.
...
Finalmente, se tendrá un conjunto de coeficientes para cada bloque que considera una constante y tres valores que ponderan las posiciones espaciales \(x\), \(y\) y \(z\). Como la metodología propuesta se enfoca únicamente en la dinámica de los atributos a través de las posiciones relativas, el término constante \(\beta_0\) no es relevante para este análisis y se omite. De este modo, cada bloque \(i\) está representado por un vector de coeficientes (pendiente):
...
donde cada fila corresponde a los coeficientes \((\beta_1, \beta_2, \beta_3)\) asociados a un bloque, en coherencia con la expresión general \(\hat{y} = \beta^\top \Phi\). 

\subsubsection{Estrategia de Entrenamiento y Ejecución}

La metodología opera en dos fases distintas:

\begin{enumerate}
    \item \textbf{Fase de Entrenamiento (Offline):} Se selecciona un subconjunto representativo de bloques (conjunto de entrenamiento) para ejecutar el algoritmo iterativo de RD-Clustering. El resultado de esta fase es un \textbf{libro de códigos optimizado} $\{\boldsymbol{\theta}_j\}_{j=0}^{K-1}$ que captura las dinámicas de señal predominantes en la nube de puntos.
    
    \item \textbf{Fase de Ejecución (Online):} Con el libro de códigos fijo, el resto de los bloques de la nube se codifican en un solo paso (single-pass). Para cada bloque, el sistema simplemente selecciona el cluster $j$ que ofrece el menor costo RD $J_j$, construye el grafo correspondiente y aplica la GFT. Esta separación garantiza que el costo computacional de la optimización evolutiva solo se pague una vez por secuencia.
\end{enumerate}

La función de costo a minimizar para cada bloque \(i\) al asignarlo al cluster \(j\) es:

\begin{equation}
    J_j(i) = D_j(i) + \lambda R_j(i)
\end{equation}

\subsubsection{Optimización Conjunta de Parámetros}

Una innovación fundamental de esta metodología es que el problema de optimización no se limita únicamente a las pendientes de color ($\mathbf{c}_j$), sino que se extiende a los parámetros de construcción del grafo. Cada cluster $j$ está definido por un vector de parámetros de 5 dimensiones:

\begin{equation}
    \boldsymbol{\theta}_j = [S_{x,j}, S_{y,j}, S_{z,j}, SLW_j, SLP_j]
\end{equation}

donde $SLW$ es el peso del auto-lazo y $SLP$ es el porcentaje de nodos seleccionados para incluir dicha conexión. Esta optimización conjunta permite que el sistema aprenda no solo la "dirección" del color, sino la topología de grafo más eficiente para compactar dicha señal.

\subsubsection{Algoritmo de Optimización Evolutiva}

El algoritmo procede de manera iterativa mediante los siguientes pasos:

\paragraph{Inicialización:} Se definen \(K\) centroides iniciales. El primer centroide (\(j=0\)) es estructural (\(\mathbf{c}_0 = \mathbf{0}\)). Los restantes \(K-1\) se inicializan mediante k-means sobre las pendientes aproximadas \(\tilde{\boldsymbol{\beta}}\).

\paragraph{Asignación y Refinamiento:} En cada iteración:
\begin{enumerate}
    \item \textbf{Paso de Asignación:} Cada bloque se asigna al cluster $j$ que minimiza $J_j(i)$ mediante un esquema de \textit{Simulated Annealing}.
    \item \textbf{Paso de Optimización:} Se utiliza un algoritmo de \textbf{Evolución Diferencial (Differential Evolution)} para actualizar $\boldsymbol{\theta}_j$ buscando minimizar el costo agregado de los bloques asignados a dicho cluster. Este proceso permite una exploración global del espacio no convexo de los parámetros de auto-lazo.
\end{enumerate}

\paragraph{Criterios de Parada (Sliding-Window Early Stopping):} Para evitar iteraciones redundantes y asegurar la estabilidad, se implementa un mecanismo de parada temprana basado en una ventana deslizante de las últimas 3 iteraciones. El sistema se detiene si:
\begin{itemize}
    \item \textbf{Convergencia de Costo:} La diferencia relativa máxima del costo RD en la ventana es menor a $10^{-4}$.
    \item \textbf{Convergencia de Asignación:} El promedio de cambios de etiquetas (distancia Hamming) es inferior al 0.5\% del total de bloques.
\end{itemize}

\subsubsection{GFT Adaptativa y Construcción de Grafos}

Cada centroide \(\mathbf{c}_j\) define una transformada de Fourier sobre grafos (GFT) específica:

\begin{itemize}
    \item \textbf{Centroide estructural} (\(\mathbf{c}_0 = \mathbf{0}\)): Corresponde a una GFT basada únicamente en la geometría, donde las aristas del grafo se ponderan según las distancias euclidianas entre puntos.
    
    \item \textbf{Centroides dinámicos} (\(\mathbf{c}_j, j \geq 1\)): Definen grafos basados en atributos donde la estructura se adapta a la tendencia de la señal representada por la pendiente \(\mathbf{c}_j\). Se añaden auto-bucles (self-loops) a los nodos que actúan como sumideros en el campo de gradiente definido por \(\mathbf{c}_j\), lo cual mejora la compactación de energía en el dominio espectral.
\end{itemize}

Esta adaptación permite que la GFT capture de manera más eficiente la estructura de correlación presente en bloques con diferentes patrones de variación de luminancia, resultando en una mejor compactación espectral y, consecuentemente, en menores tasas de bits para un nivel de distorsión dado. El detalle respecto a la construcción del grafo basado en atributos se presenta a continuación.

\section{Construcción del Grafo de Atributos}\label{sec:attribute_graph}
    ...
    \subsection{Matriz de dinámica de atributos}
        Con el apropósito de visualizar la dinámica del atributo entre vecinos. La matriz de atributos se define como:
        \begin{equation}
    ...
    \subsection{Selección de \textit{self-loops}}
        ...
        Finalmente la selección de los nodos estratégicos se realiza a través de un umbral estático por sobre todos los bloques evaluado en el vector de sumidero. El objetivo será privilegiar la implementación de \textit{self-loop} en aquellos puntos que presentan una influencia mayor en el decaimiento del atributo. Para ello, se utiliza un umbral $\tau \in [0,1]$ determinado por análisis de sensibilidad, que representa la relevancia suficiente para que un nodo incluya un \textit{self-loop}. 

        \begin{equation}
            \mathcal{S} = \left\{ i \in \{1,\dots,N\} \mid S_i \geq \tau \right\}
        \end{equation}

        Sobre este conjunto $\mathcal{S}$, se agregan conexiones representadas en la diagonal de la matriz de adyacencia con un valor fijo $w_{sl}$ que se ve de la siguiente manera:

        \begin{equation}
            W_{ii} \leftarrow w_{\text{sl}}, \quad \forall i \in \mathcal{S}
        \end{equation}

        Tanto el umbral $\tau$ como el peso $w_{\text{sl}}$ se encuentran mediante análisis de sensibilidad sobre un conjunto de entrenamiento pequeño de cada secuencia.
...
\section{Transformada Gráfica de Fourier}\label{sec:method_gft}
    ...
    Para el grafo estructural la primera base del Laplaciano $L = D - A$ está asociado al valor propio $\lambda_1 = 0$ (vease Sec. \ref{subsec: gft}) y satisface:
...
\section{Optimización Tasa-Distorsión}\label{sec:rdo_method}
   ...
   En este sentido, la implementación presenta un conjunto de desafíos a resolver para poder hacer uso práctido de la expresión anterior. Ya que considera elementos que no son determinables a priori y por tanto es necesario encontrar expresiones aproximadas de sus valores.
    ...
    Es por aquello, que definimos la distorsión aportada a la ecuación de optimización considerando el bloque $i$ y paso de cuantización $j$ de la forma:
...
\section{Codificación Entrópica}\label{sec:rlgr_method}
...
\section{\textit{Overhead} de Decisión y Señalización}\label{sec:overhead_method}

El \textit{overhead} corresponde a la porción del flujo de bits reservada para las instrucciones de adaptación. En esta metodología, esto implica transmitir el índice del cluster $j$ seleccionado para cada bloque. 

\subsection{Codificación por Entropía de Índices}

A diferencia de una representación binaria simple de longitud fija, se utiliza la entropía de la distribución de clusters para estimar el costo mínimo de señalización. La entropía del libro de códigos $H$ se define como:

\begin{equation}
    H = -\sum_{j=0}^{K-1} p_j \log_2(p_j)
\end{equation}

donde $p_j$ es la probabilidad (frecuencia relativa) de que un bloque sea asignado al cluster $j$. El costo total en bits para señalizar las decisiones de $N_{bloques}$ es:

\begin{equation}
    B_{\text{decisión}} = N_{bloques} \cdot H
\end{equation}

\subsection{Capacidad Efectiva del Libro de Códigos}

Un concepto clave para el análisis de eficiencia es la \textbf{Capacidad Efectiva} ($K_{eff}$), calculada como $2^H$. Esta métrica representa el número real de clusters que están contribuyendo significativamente a la compresión. Si $K_{eff} \ll K$, indica que el sistema ha convergido a una representación más compacta que la capacidad física permitida, sugiriendo un potencial para la reducción del overhead mediante la poda de clusters subutilizados.
...
\section{Etapa de Decodificación}\label{sec:decode_method}
   ...
   \subsection{Conversión al Espacio de Color Final (YUV a RGB)}
   ...
   \subsection{Error de Reconstrucción} \label{sec:metrics_method}
