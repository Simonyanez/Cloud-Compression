...
Las \textit{nubes de puntos 3D} son una representación eficiente y flexible para describir objetos, personas y espacios interiores o exteriores. Estas representaciones tienen aplicaciones clave en áreas como la realidad aumentada y virtual (AR/VR), la telepresencia y los vehículos autónomos. Una nube de puntos 3D consiste en una lista de puntos que indican posiciones en el espacio, y una serie de atributos asociados, siendo el más común el color.
\vspace{3mm}
Formalmente, la geometría de la nube se puede representar mediante una matriz $V \in \mathbb{R}^{n \times 3}$, donde cada fila $v_i = [x_i, y_i, z_i]$ corresponde a las coordenadas 3D de un punto. A su vez, los atributos de color se representan mediante otra matriz $C \in \mathbb{R}^{n \times 3}$, donde cada fila $c_i = [r_i, g_i, b_i]$ contiene la información de color RGB del punto correspondiente.
\vspace{3mm}
Gracias al avance tecnológico reciente, este tipo de datos puede ser capturado en alta resolución, en tiempo real y a bajo costo, utilizando por ejemplo sensores \textit{LiDAR} integrados en teléfonos móviles o vehículos autónomos. 
\vspace{3mm}
Esta facilidad de adquisición ha llevado a la generación de enormes volúmenes de datos 3D, lo cual ha motivado el desarrollo de técnicas de compresión eficiente, tanto para la geometría como para los atributos de color de las nubes de puntos~\cite{schwarz2018emerging}.
\vspace{3mm}
En imágenes y video convencionales, la compresión con pérdida se realiza de manera eficiente mediante procesamiento por bloques y transformadas como la \textit{Discrete Cosine Transform} (DCT), usada en estándares como \textit{JPEG}. Sin embargo, las nubes de puntos presentan geometría irregular, desordenada y sin estructura temporal consistente, lo que impide aplicar directamente técnicas tradicionales.
\vspace{3mm}
Frente a este desafío, el procesamiento de señales sobre grafos (\textit{Graph Signal Processing}) ha emergido como una herramienta poderosa para trabajar con datos no estructurados, como las nubes de puntos~\cite{shuman2013emerging, ortega2018graph}. En este trabajo se propone una estrategia basada en GSP para la compresión con pérdida de los \textit{atributos de color} de nubes de puntos 3D.
